...
class MessageController extends Controller {
  async count() {
    ...
    const count = ctx.service.message.getMessagesCount(userId);
  ...
  async markAll() {
    const { ctx } = this;
    const userId = ctx.request.user._id;
    const messages = ctx.service.message.getUnreadMessagesById(userId);
    const result = ctx.service.message.updateMessageToRead(userId, messages);
