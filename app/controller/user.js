...
class UserController extends Controller {
  ...
  async listCollectedTopics() {
    ...
    const limit = this.config.list_topic_count || 20;
    ...
    const opt = { skip: (pages - 1) * limit, limit };

    const collects = await service.topicCollect.getTopicCollectsByUserId(user.id, opt);
  ...
  async listTopics() {
    const { ctx, service, config } = this;
    const user_name = ctx.params.name; // 这个是匹配路径参数
    const page = Number(ctx.query.page) || 1; // 这个是? 后面的参数
    const limit = config.list_topic_count || 20;
  ...
  async listReplies() {
    ...
    const user = service.user.getUserByLoginName(user_name);
    if (!user) {
      ctx.status = 404;
      ctx.message = '这个用户不存在。';
      return;
    ...
    let topics = await service.topic.getTopicByQuery(query, {});
  ...
  async block() {
    const { ctx, service } = this;
    const action = ctx.request.body.action; // 这个从哪里获取的？
